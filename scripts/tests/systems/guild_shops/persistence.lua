...
-- Runs the restore and collect helper against xi.guildShops.state directly.
-- xi_test does not load modules/init.txt, so the helper is required here.
-----------------------------------
require('modules/phoenix/lua/custom/guild_shop_persistence')

describe('Guild shop persistence', function()
    ---@type CClientEntityPair
    local player

    local shopName    = 'Kamilah'
    local probe       = xi.item.CHUNK_OF_TIN_ORE
    local persistence = xi.guildShops.persistence
    ...
    -- Other suites leave their own shops in state, so only count this shop's deltas.
    local function deltasFor(deltas)
        local mine = {}
    ...
    it('restores a covered snapshot with its prices and lastRoll intact', function()
        ...
        assert(result.reRolled == 0, 'covered shop was set to re-roll')
    ...
    it('re-rolls a shop that gained a config item, keeping stock without restocking', function()
        ...
        assert(result.reRolled == 1, 'uncovered shop not set to re-roll')
        ...
        assert(state.items[probe].stock == stock, 'first roll restocked the restored stock')
        assert(state.items[probe].buyPrice ~= saved.buyPrice, 'first roll kept the saved price')
        assert(state.items[missing.id].stock == missing.initial, 'new config item not seeded')
    end)
    ...
    it('reports saved rows for items no longer in the config', function()
        ...
        assert(result.restored == 1, 'extra row stopped the shop restoring')
    ...
    it('collects only stock changes until a roll, which returns every row', function()
        ...
        assert(#deltasFor(persistence.collect()) == 0, 'restore alone produced deltas')

        local state = xi.guildShops.state[shopName]
        state.items[probe].stock = state.items[probe].stock - 1

        local deltas = deltasFor(persistence.collect())
        assert(#deltas == 1, 'stock change did not produce exactly one delta')
        assert(deltas[1].itemId == probe and deltas[1].stock == state.items[probe].stock, 'wrong delta')
        assert(#deltasFor(persistence.collect()) == 0, 'delta reported twice')

        state.lastRoll = state.lastRoll + 1
        assert(#deltasFor(persistence.collect()) == #configItems(), 'roll did not flush every row')
    end)
end)
