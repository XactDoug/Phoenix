...
private:
    ...
    void restore()
    {
        ...
        for (const auto& [key, value] : deletes)
        {
            ...
            if (db::preparedStmt("DELETE FROM guild_shop_state WHERE shop = ? AND itemid = ?",
                                 entry.get<std::string>("shop"),
                                 entry.get<uint16>("itemId")))
            {
                ++pruned;
            }
            else
            {
                ...
                ShowErrorFmt("Guild shop persistence: failed to prune stale row {}/{}", entry.get<std::string>("shop"), entry.get<uint16>("itemId"));
            }
        }
